...
import { AREA, ROOT, UPSTREAM, safePath, linkedVault, readJson, atomicWrite } from './paths.mjs';
...
import { openInObsidian } from './open.mjs';

const help = `Research Studio — existing tools, one entry point

/studio                         Open a fresh Pi research session
/studio vault "vault path"       Link/relink a vault and install the bundled Obsidian viewer
/studio open                    Open this conversation in Obsidian
/studio example                 Open the interactive example in your linked vault
...
Answers open inside Obsidian, with interactive visuals, searchable text and linked sources.
...
For substantial explanations, comparisons and research, produce a complete visual report by default, unless the user requests text only. For a simple fact or short follow-up, a direct text answer is enough. Read ${join(UPSTREAM.visual, 'SKILL.md')} and relevant references using read. Use studio_visual_explainer_prepare and studio_visual_explainer_render_html to make a complete dark HTML report containing the explanation, useful tables/charts/diagrams, and source links. Studio publishes the result as an Obsidian note automatically; use open:false. Never launch a browser, local server, or external HTML viewer. The user reads answers inside Obsidian. Prefer full HTML; quick mode is only for explicitly requested small visuals. Use a charcoal Obsidian-like background, system fonts, restrained color, generous spacing, readable labels, responsive charts and no nested scroll boxes. Give every chart an explanatory takeaway. Choose diagrams to explain mechanisms and tables to compare like-for-like evidence; avoid decorative charts. Use self-contained HTML with inline CSS and SVG/canvas scripts. If needed, load Mermaid or Chart.js from cdn.jsdelivr.net; other external scripts, fonts, stylesheets and fetch requests are blocked by the Obsidian viewer. Use pre.mermaid or pre > code.language-mermaid for Mermaid. Preserve substantive prose, captions and static data tables in the initial HTML even when scripts fill a chart. These become the searchable Markdown fallback; never rely on JavaScript to create the explanation or the only copy of the data. Reconcile figure numbers, units, dates and citations against source data. Do not fabricate data or claim rendered verification without actually viewing it.
...
Save generated research files only under ${join(root, AREA)}. Visual Explainer's configured output directory is ${join(root, AREA, 'Reports')}; that overrides upstream example paths. Link the returned Markdown note path in the final message. The conversation embeds that note automatically; keep the final Pi message brief once the full answer is published. Pi transcripts and submitted images are archived automatically. Do not edit Studio sessions or extension code.
...
export function registerStudio(pi, { loadWeb, loadRuntime, research = runResearch, openNote = openInObsidian, makeMcp = (...args) => new LocalMcp(...args) } = {}) {
      ...
      try {
        ...
        for (const definition of definitions) {
          ...
          pi.registerTool({ name, label: definition.title || 'Visual Explainer', description: definition.description || definition.name, parameters: definition.inputSchema,
            execute: async (_id, args, signal, _update, toolCtx) => {
              if (!active) throw new Error('Open /studio first.');
              const renders = ['visual_explainer_render_html', 'visual_explainer_render_quick'].includes(definition.name);
              // Every published answer owns its asset; later renders cannot replace old answers.
              const parameters = renders ? { ...args, filename: `answer-${randomUUID()}`, open: false } : args;
              const result = await mcp.request('tools/call', { name: definition.name, arguments: parameters }, signal);
              if (result.isError) throw new Error(result.content.filter(item => item.type === 'text').map(item => item.text).join('\n'));
              if (renders) {
                const conversation = await archiveConversation(root, toolCtx.sessionManager);
                const report = await publishReport(root, result.structuredContent.path, conversation);
                pi.appendEntry('research-studio-report', report);
                await archiveConversation(root, toolCtx.sessionManager);
                return { content: [{ type: 'text', text: `Published the full answer in Obsidian: ${join(root, report.notePath)}\nLinked conversation: ${conversation}\nThe note includes the interactive report, a readable Markdown copy and linked sources. Obsidian opens after the answer finishes. Do not open the HTML in a browser.` }], details: report };
              }
  ...
  pi.registerCommand('studio', {
    ...
    getArgumentCompletions(prefix) {
      const descriptions = { vault: 'Link your Obsidian vault', open: 'View this conversation in Obsidian', example: 'Interactive example in Obsidian, no model call', here: 'Activate in this chat', ask: 'Focused answer in Pi', deep: 'Deep research in Pi', web: 'Upstream web settings/commands', doctor: 'Check installation', exit: 'Leave Studio', help: 'Show every command' };
    ...
    async handler(args, ctx) {
      ...
      try {
        ...
        if (command === 'vault') {
          ...
          return notify(ctx, `Linked ${vault}. The Research Studio Viewer is installed. If Obsidian is already open, restart it once; allow community plugins if Restricted mode is on. Run /studio example to check the presentation, or /studio ask to begin. Pi Agent remains optional.`);
        }
        if (command === 'example') {
          const vault = await linkedVault();
          const html = await safePath(vault, `${AREA}/Reports/example-${randomUUID()}.html`);
          await atomicWrite(html, await readFile(join(ROOT, 'examples/growth.html')));
          const report = await publishReport(vault, html);
          await openNote(await safePath(vault, report.notePath));
          return notify(ctx, 'Opened the example note in Obsidian. Its data is hypothetical; no model call was made.');
        }
        if (command === 'open') {
          if (!active) throw new Error('Open /studio first, or use /studio example.');
          const note = await archiveConversation(root, ctx.sessionManager);
          await openNote(note); return notify(ctx, `Opened in Obsidian: ${note}`);
        }
        if (command === 'exit') {
  ...
  pi.on('agent_end', async (_event, ctx) => {
    ...
    clearTimer(ctx);
    try {
      const path = await archiveConversation(root, ctx.sessionManager);
      notify(ctx, `Saved answer · ${Math.round((Date.now() - startedAt) / 1000)}s\n${path}`);
      if (ctx.hasUI) {
        try { await openNote(path); }
        catch (error) { notify(ctx, `The answer is saved, but Obsidian could not open: ${error.message}. Open the note above manually.`, 'warning'); }
      }
    }
